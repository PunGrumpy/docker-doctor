...
- 8bded00: `no-docker-socket-mount` compared the volume source against the literal `/var/run/docker.sock`, so it caught 1 of the 5 ways agent stacks mount the socket. The rule now resolves `${VAR:-default}` interpolation to the default and matches the socket by name on any path-shaped source, which covers:
...
- fed7f98: `require-resource-limits` only looked at `deploy.resources.limits`, so it warned services that set limits through the service-level `mem_limit`, `cpus`, or `cpu_quota` keys. Those keys are the older spelling of the same limits, still in the Compose specification and applied the same way by `docker compose`, and now satisfy the rule.
