...
export const rulePageContent: Record<string, RulePageContent> = {
  ...
  "docker-doctor/no-secrets-in-env": {
    ...
    intro:
      "`ENV API_KEY=...` feels like configuration, but it is storage: the value is written into the image's metadata and distributed with every push and pull. `ARG` is no safer — build arguments are recorded in the layer history. Anyone who can pull the image can read them back with `docker history --no-trunc` or `docker inspect`. This rule flags `ENV` and `ARG` keys that look like credentials (`PASSWORD`, `SECRET`, `TOKEN`, `API_KEY`, …) with literal values. A URL without embedded credentials (`AUTH_URL=http://auth:8080`) names an endpoint, not a secret, so the rule skips it.",
    notes:
  ...
  "docker-doctor/pin-model-version": {
    ...
    intro:
      "The top-level `models:` element (Docker Model Runner, Compose ≥ 2.35) declares AI models as OCI artifacts, and OCI rules apply: `model: ai/gemma3` with no tag resolves to whatever the registry currently serves. For a model, drift means _different weights_: often a different parameter count or quantization, with different memory needs and different behavior on the same prompts. This rule flags top-level models whose `model:` reference has no tag or uses `latest`, and applies the same check to the earlier service-level `provider: { type: model }` syntax.",
    notes:
