...
// Whether the folder holds the current page — nested folders open on it.
...
const SidebarNode = ({ node, pathname, depth }: SidebarNodeProps) => {
  ...
  if (node.type === "folder") {
    ...
    // Top-level sections stay always-visible headings; nested folders
    // (e.g. the 25-rule "Rules" group) collapse behind a native
    // disclosure so they stop drowning out the rest of the tree.
    if (depth === 0) {
      return (
    ...
    return (
      <details className="group" open={containsPath(node, pathname)}>
        ...
        {/* One subordination step per level, on the TOC's dashed-rail idiom. */}
...
export const DocsSidebar = ({ tree }: DocsSidebarProps) => {
  ...
  // Deep links can land on an entry far down the scrollable sidebar; bring
  // the active link into view so "where am I" is visible on arrival.
  useEffect(() => {
    navRef.current
