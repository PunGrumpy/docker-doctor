...
The tag is required — sbx does not default to `:latest` for registry kits. Pin a specific release instead if you want reproducible sandboxes: the kit is tagged with the CLI version it installs, e.g. `docker.io/pungrumpy/docker-doctor-kit:0.4.0`.
