...
Prints only the numeric score. The exit code follows the same rule as every other mode: non-zero when any `error`-severity diagnostic is present, so a CI gate behaves identically whichever output you pick.
