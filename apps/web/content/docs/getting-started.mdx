...
- Browse every rule with `npx @docker-doctor/cli@latest rules list`, or read the full list in [Rules](/docs/rules).
- See all flags and exit codes in the [CLI reference](/docs/cli).
- Tune rule severities or ignore files in [Configuration](/docs/configuration).
- Understand how the health score is calculated in [Scoring](/docs/scoring).
